        ...
        <TouchableWithoutFeedback className="w-full h-full" onPress={Keyboard.dismiss}>
            <View className="w-full h-full flex flex-col">
                ...
                <View className={`w-full ${isKeyboardVisible && Platform.OS != 'ios' ? 'h-[70%]' : 'h-[50%]'} flex flex-col items-center justify-center relative`}>
                    <Text className="text-[#4b4b4b] text-3xl top-3 absolute">Logga in</Text>
                    <TextInput className={`w-[80%] ${Platform.OS == 'ios' ? 'h-[12%]' : 'h-14'} text-center border-[.1rem] border-[#D1D1D1] rounded-[.9rem] top-[7rem] absolute`} placeholder='E-post' onChangeText={(value) => setEmail(value)} value={email}/>
                    <TextInput className={`w-[80%] ${Platform.OS == 'ios' ? 'h-[12%]' : 'h-14'} text-center border-[.1rem] border-[#D1D1D1] rounded-[.9rem] top-[12rem] absolute`} placeholder='Lösenord' secureTextEntry={true} onChangeText={(value) => setPassword(value)} value={password}/>
                    <Pressable className={`w-[80%] ${Platform.OS == 'ios' ? 'h-[12%]' : 'h-14'} flex items-center justify-center text-center bg-[#EA6300] rounded-[.9rem] top-[19rem] absolute`} onPress={() => dispatch(login({email, password}))}>
                        <Text className="text-white">Logga in</Text>
                    </Pressable>
                    <Pressable className={`w-auto h-auto flex items-center justify-center text-center rounded-[.9rem] top-[23rem] absolute`} onPress={() => navigation.navigate('Signup')}>
                        <Text className="text-[#000000] text-m">Har du inget konto? Registrera dig</Text>
                    </Pressable>
                </View>
            </View>
